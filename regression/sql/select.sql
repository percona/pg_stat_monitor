...
PREPARE pgsm_test (int) AS SELECT $1, 'test' LIMIT 1;
EXECUTE pgsm_test(1);
DEALLOCATE pgsm_test;
...
--
CREATE TABLE pgsm_a (id integer PRIMARY KEY);
CREATE TABLE pgsm_b (id integer PRIMARY KEY, a_id integer REFERENCES pgsm_a);

SELECT pg_stat_monitor_reset() IS NOT NULL AS t;

-- control query
SELECT * FROM pgsm_a JOIN pgsm_b ON pgsm_b.a_id = pgsm_a.id;

-- test range tables
SELECT * FROM pgsm_a JOIN pgsm_b ON pgsm_b.a_id = pgsm_a.id FOR UPDATE;
SELECT * FROM pgsm_a JOIN pgsm_b ON pgsm_b.a_id = pgsm_a.id FOR UPDATE OF pgsm_a;
SELECT * FROM pgsm_a JOIN pgsm_b ON pgsm_b.a_id = pgsm_a.id FOR UPDATE OF pgsm_b;
SELECT * FROM pgsm_a JOIN pgsm_b ON pgsm_b.a_id = pgsm_a.id FOR UPDATE OF pgsm_a, pgsm_b; -- matches plain "FOR UPDATE"
SELECT * FROM pgsm_a JOIN pgsm_b ON pgsm_b.a_id = pgsm_a.id FOR UPDATE OF pgsm_b, pgsm_a;

-- test strengths
SELECT * FROM pgsm_a JOIN pgsm_b ON pgsm_b.a_id = pgsm_a.id FOR NO KEY UPDATE;
SELECT * FROM pgsm_a JOIN pgsm_b ON pgsm_b.a_id = pgsm_a.id FOR SHARE;
SELECT * FROM pgsm_a JOIN pgsm_b ON pgsm_b.a_id = pgsm_a.id FOR KEY SHARE;

-- test wait policies
SELECT * FROM pgsm_a JOIN pgsm_b ON pgsm_b.a_id = pgsm_a.id FOR UPDATE NOWAIT;
SELECT * FROM pgsm_a JOIN pgsm_b ON pgsm_b.a_id = pgsm_a.id FOR UPDATE SKIP LOCKED;

SELECT calls, query FROM pg_stat_monitor ORDER BY query COLLATE "C";

DROP TABLE pgsm_a, pgsm_b CASCADE;
SELECT pg_stat_monitor_reset() IS NOT NULL AS t;
...
SELECT COUNT(*) FROM pg_stat_monitor WHERE query LIKE '%FROM%tbl_inh%';
...
SELECT COUNT(*) FROM pg_stat_monitor WHERE query LIKE '%FETCH FIRST%';
...
SELECT calls, query FROM pg_stat_monitor WHERE query LIKE '%GROUP BY%' ORDER BY query COLLATE "C";
...
SELECT COUNT(*) FROM pg_stat_monitor WHERE query LIKE '%GROUP BY%ROLLUP%';
...
SELECT COUNT(*) FROM pg_stat_monitor WHERE query LIKE '%SELECT GROUPING%';
SELECT pg_stat_monitor_reset() IS NOT NULL AS t;
...
CREATE SCHEMA pgsm_schema_1;
CREATE SCHEMA pgsm_schema_2;
-- Same attributes.
CREATE TABLE pgsm_schema_1.tab_search_same (a int, b int);
CREATE TABLE pgsm_schema_2.tab_search_same (a int, b int);
CREATE TEMP TABLE tab_search_same (a int, b int);
-- Different number of attributes, mapping types
CREATE TABLE pgsm_schema_1.tab_search_diff_1 (a int);
CREATE TABLE pgsm_schema_2.tab_search_diff_1 (a int, b int);
CREATE TEMP TABLE tab_search_diff_1 (a int, b int, c int);
-- Same number of attributes, different types
CREATE TABLE pgsm_schema_1.tab_search_diff_2 (a int);
CREATE TABLE pgsm_schema_2.tab_search_diff_2 (a text);
CREATE TEMP TABLE tab_search_diff_2 (a bigint);
-- First permanent schema
SET search_path = 'pgsm_schema_1';
...
SELECT count(*) FROM pgsm_schema_1.tab_search_same;
SELECT a, b FROM pgsm_schema_1.tab_search_same;
SELECT count(*) FROM pgsm_schema_2.tab_search_diff_1;
SELECT count(*) FROM pg_temp.tab_search_diff_2;
SELECT a FROM pgsm_schema_2.tab_search_diff_2 AS t1;
SELECT a FROM pgsm_schema_2.tab_search_diff_2;
SELECT a AS a1 FROM pgsm_schema_2.tab_search_diff_2;
SELECT calls, query FROM pg_stat_monitor ORDER BY query COLLATE "C";
DROP SCHEMA pgsm_schema_1 CASCADE;
DROP SCHEMA pgsm_schema_2 CASCADE;
