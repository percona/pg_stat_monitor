...
-- Unlike upstream pg_stat_statements, pg_stat_monitor does not track the
-- execution of DEALLOCATE, so no entry is reported for it below.
SELECT 1 as a;
...
-- The statements issued under SET SESSION AUTHORIZATION above are attributed
-- to the role they ran as, so some of the entries below are split per user.
-- The user names are deliberately not selected here, as the name of the
-- bootstrap superuser depends on the environment.
SELECT calls, rows, query FROM pg_stat_monitor ORDER BY query COLLATE "C";
...
--
CREATE TABLE pgsm_ctas AS SELECT a, 'ctas' b FROM generate_series(1, 10) a;
SELECT generate_series(1, 10) c INTO pgsm_select_into;
COPY pgsm_ctas (a, b) FROM STDIN;
11	copy
12	copy
13	copy
\.
CREATE MATERIALIZED VIEW pgsm_matv AS SELECT * FROM pgsm_ctas;
REFRESH MATERIALIZED VIEW pgsm_matv;
BEGIN;
DECLARE pgsm_cursor CURSOR FOR SELECT * FROM pgsm_matv;
FETCH NEXT pgsm_cursor;
FETCH FORWARD 5 pgsm_cursor;
FETCH FORWARD ALL pgsm_cursor;
COMMIT;

SELECT calls, rows, query FROM pg_stat_monitor ORDER BY query COLLATE "C";

DROP MATERIALIZED VIEW pgsm_matv;
DROP TABLE pgsm_ctas;
DROP TABLE pgsm_select_into;
