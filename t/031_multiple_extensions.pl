...
if (index(lc($PG_VERSION_STRING), lc("percona")) == -1 && index(lc($PG_VERSION_STRING), lc("pgdg")) == -1)
{
    plan skip_all => "pg_stat_monitor test case only for PGDG or PPG server package install with extensions.";
}
