...
($cmdret, $stdout, $stderr) = $node->psql(
	'postgres',
	'\i scripts/data_1.sql',
	extra_params => [ '-a', '-Pformat=aligned', '-Ptuples_only=off' ]);
is($cmdret, 0, "Run sql file:  scripts/data.sql");
#PGSM::append_to_file($stdout);
...
($cmdret, $stdout, $stderr) = $node->psql(
	...
	extra_params => [ '-a', '-Pformat=aligned', '-Ptuples_only=off' ]);
is($cmdret, 0,
	"SELECT substr(query, 0, 50), calls, message FROM pg_stat_monitor");
PGSM::append_to_file($stdout);
...
($cmdret, $stdout, $stderr) = $node->psql(
	'postgres',
	'\i scripts/data_2.sql',
	extra_params => [ '-a', '-Pformat=aligned', '-Ptuples_only=off' ]);
is($cmdret, 0, "Run sql file:  scripts/data2.sql");
#PGSM::append_to_file($stdout);
