...
our @EXPORT =
  qw( pgsm_init_pg pgsm_start_pg pgsm_stop_pg pgsm_psql_cmd pgsm_setup_pg_stat_monitor pgsm_create_extension pgsm_reset_pg_stat_monitor pgsm_drop_extension );
...
BEGIN
{
	...
	my $node_module =
	  $PG_MAJOR_VERSION > 14 ? "PostgreSQL::Test::Cluster" : "PostgresNode";
...
sub append_to_file
{
	...
	if ($PG_MAJOR_VERSION > 14)
	{
		PostgreSQL::Test::Utils::append_to_file($out_filename_with_path,
			$str . "\n");
	}
...
sub append_to_debug_file
{
	...
	if ($PG_MAJOR_VERSION > 14)
	{
		PostgreSQL::Test::Utils::append_to_file($debug_out_filename_with_path,
			$str . "\n");
	}
	# For Server version 14 & below, use PostgresNode to write to files
	else
...
sub setup_files_dir
{
	...
	if ($filename_without_extension eq
		"007_settings_pgsm_query_shared_buffer")
	{
		$expected_filename .= ".$PG_MAJOR_VERSION";
	}
